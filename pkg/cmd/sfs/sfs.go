...
import (
	...
	"novachat_engine/pkg/cmd/sfs/server"
	"novachat_engine/pkg/cmd/sfs/server/rpc"
...
func (m *Application) RunLoop() error {
	...
	impl := rpc.NewSfsServiceImpl(&m.conf)

	handler := server.NewSfsHandler(&m.conf)
	handler.Install()
	sfsService.RegisterSfsServiceServer(m.rpcServer, impl)
