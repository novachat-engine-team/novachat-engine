...
import (
	...
	"novachat_engine/pkg/cmd/relay/rpc"
...
func (m *Application) RunLoop() error {
	...
	var err error
	m.server, err = server.NewServer(&m.conf)
	if err != nil {
	...
	relayService.RegisterRelayServiceServer(m.rpcServer, rpc.NewServiceImpl(m.server))
