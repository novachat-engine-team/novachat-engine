 ...
 * @File : phone.acceptCall_handler.go
 * @Desc :
 *
 */
...
func (s *PhoneServiceImpl) PhoneAcceptCall(ctx context.Context, request *mtproto.TLPhoneAcceptCall) (*mtproto.Phone_PhoneCall, error) {
	...
	log.Debugf("PhoneAcceptCall %v, request: %v", metadata.RpcMetaDataDebug(md), request)

	panic("PhoneAcceptCall")
	//replyPhoneCall, err := s.phoneCallCore.AcceptPhoneCall(md.UserId, md.AuthKeyId, request.Peer.Id, request.GB, request.Protocol)
	//if err != nil {
	//	log.Errorf("PhoneAcceptCall - request: %v error:%s", request, err.Error())
	//	return nil, err
	//}
	//
	//log.Infof("PhoneAcceptCall - request: %v ok reply!!!!!!!!!!!!!!!!!!!!!!!!!", request)
