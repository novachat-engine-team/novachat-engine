 ...
 * @File : messages.deleteChatUser_handler.go
 * @Desc :
 *
 */
...
-------------- update -------------
Recv: { core_message
  msg_id: 6990261410360120321 [LONG],
  seq_no: 165 [INT],
  bytes: 544 [INT],
...
func (s *MessagesServiceImpl) MessagesDeleteChatUser(ctx context.Context, request *mtproto.TLMessagesDeleteChatUser) (*mtproto.Updates, error) {
	...
	log.Infof("MessagesDeleteChatUser %v, request: %v", metadata.RpcMetaDataDebug(md), request)

	addUserResp, err := chatService.GetChatClientByKeyId(constants.PeerTypeFromChatIDType32(request.ChatId).ToInt()).ReqDeleteUser(
		ctx,
	...
	if err != nil {
		log.Errorf("MessagesDeleteChatUser %v, request: %v error:%s", metadata.RpcMetaDataDebug(md), request, err.Error())
		return nil, err
	}

	var updates mtproto.Updates
	types.UnmarshalAny(addUserResp, &updates)
	log.Infof("MessagesDeleteChatUser %v, request: %v ok!!!!!!!!!!!!!!!!!", metadata.RpcMetaDataDebug(md), request)
