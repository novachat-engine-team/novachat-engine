 ...
 * @File : channels.getFullChannel_handler.go
 * @Desc :
 *
 */
...
func (s *ChannelsServiceImpl) ChannelsGetFullChannel(ctx context.Context, request *mtproto.TLChannelsGetFullChannel) (*mtproto.Messages_ChatFull, error) {
	...
	log.Infof("ChannelsGetFullChannel %v, request: %v", metadata.RpcMetaDataDebug(md), request)
	...
	if err != nil {
		log.Errorf("ChannelsGetFullChannel %v, request: %v error:%s", metadata.RpcMetaDataDebug(md), request, err.Error())
		return nil, err
	}
	...
	if err != nil {
		log.Warnf("ChannelsGetFullChannel request: %v GetNotifySetting error:%s", request, err.Error())
	}
	...
	if chatData.Deleted {
		...
		conversation, err1 := s.messageCore.GetConversationByPeerList(md.UserId, []*data_message.Conversation{{PeerId: chatId, PeerType: constants.PeerTypeChat.ToInt32()}})
		if err1 != nil {
			log.Errorf("ChannelsGetFullChannel %v, request: %v GetConversationByPeerList error:%s", metadata.RpcMetaDataDebug(md), request, err1.Error())
			return nil, err1
		}
	...
	log.Infof("ChannelsGetFullChannel %v, request: %v fullChat:%+v chat:%+v", metadata.RpcMetaDataDebug(md), request, fullChat, chat)
