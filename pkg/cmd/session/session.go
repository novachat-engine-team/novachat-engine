...
import (
	...
	"novachat_engine/pkg/cmd/session/rpc"
...
func (m *Application) RunLoop() error {
	...
	m.handler = handler.NewSessionHandler(&m.conf)
	m.messageChannel = handler.NewMessageChannel(m.handler, &m.conf)

	gateClient.InstallGateClient(m.conf.GateRpcClient)
	authClient.InstallAuthClient(m.conf.AuthRpcClient)
	sessionService.RegisterSessionServiceServer(m.rpcServer, rpc.NewSessionImpl(m.handler, m.messageChannel, &m.conf))
...
func (m *Application) OnEvent(filename string) {
	...
	m.handler.SetEnableMQ(cnf.BizRpc.EnableMQ)
}
