...
func GenerateStringHash(size int) string {
	b := make([]byte, size)
	_, _ = rand.Read(b)
	return hex.EncodeToString(b)
}
...
func FormatAccessHashInt64(accessHash int64) string {
	return fmt.Sprintf("%032d", accessHash)
}

func FormatAccessHashString(accessHash string) string {
	return fmt.Sprintf("%032s", accessHash)
}
