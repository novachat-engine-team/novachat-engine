...
func (c *Core) FillMessageBox(ctx context.Context, userId int64, peerId int64, list []*msgService.SendMessageData, boxType message.BoxType) ([]int32, bool, error) {
	...
	box, err1 := c.getMessageBoxes(ctx, list[0].Message.GroupedId != 0, userId, message.MakeMessageUserId(userId, boxType), randIdList)
	if err1 != nil {
		log.Fatalf("fillMessageBox GetMessageBoxes error:%s", err1.Error())
	...
	if list[0].Message.GroupedId != 0 {
		...
		if err != nil {
			log.Errorf("fillMessageBox NextId true error:%s", err.Error())
		...
		err = c.setMessageBoxes(ctx, true, userId, message.MakeMessageUserId(userId, boxType), randIdList, messageIdList, ptsList)
		if err != nil {
			if mongo.IsDuplicateKeyError(err) {
				return ptsList, true, nil
			}
			log.Errorf("fillMessageBox SetMessageBoxes true error:%s", err.Error())
			return nil, false, err
		}
	} else {
		...
		if len(randomIdList) > 0 {
			...
			if err != nil {
				log.Errorf("fillMessageBox NextId false error:%s", err.Error())
				return nil, false, err
			}
			_idx := int32(1)
			...
			err = c.setMessageBoxes(ctx, false, userId, message.MakeMessageUserId(userId, boxType), randIdList, tempMessageIdList, tempPtsList)
			if err != nil {
				if mongo.IsDuplicateKeyError(err) {
					return tempPtsList, true, nil
				}

				log.Errorf("fillMessageBox SetMessageBoxes false error:%s", err.Error())
				return nil, false, err
			}
...
func (c *Core) getMessageBoxes(ctx context.Context, group bool, userId int64, peerId int64, randomIdList []int64) ([]*data_message.MessageBox, error) {
	...
	if group {
		idList = append(idList, fmt.Sprintf("%d%d", userId, hash.Hash64(randomIdList, len(randomIdList))))
	} else {
		for _, v := range randomIdList {
			idList = append(idList, fmt.Sprintf("%d%d", userId, v))
		}
	}

	col := mgo.GetMongoDB().Database(message.DBMessage).Collection(message.TableMessageBox, op)
	if len(idList) == 1 {
		filter := bson.M{"_id": idList[0]}
		sr := col.FindOne(ctx, filter)
		if sr.Err() != nil && sr.Err() != mongo.ErrNoDocuments {
			log.Fatalf("GetMessageBoxes error:%s", sr.Err().Error())
			return nil, sr.Err()
		}
		...
		err := sr.Decode(&box)
		if err != nil {
			log.Errorf("data_message.MessageBox decode error:%s", err.Error())
			return nil, err
		}
		if len(box.Id) == 0 {
			return nil, nil
		...
		if err != nil && err != mongo.ErrNoDocuments {
			log.Fatalf("GetMessageBoxes error:%s", err.Error())
			return nil, err
		}
...
func (c *Core) setMessageBoxes(ctx context.Context, group bool, userId int64, peerId int64, randomIdList []int64, messageList []int32, ptsList []int32) error {
	if ctx == nil {
	...
	if group == true {
		result, err := mgo.GetMongoDB().Database(message.DBMessage).Collection(message.TableMessageBox, op).
		...
		if err != nil {
			log.Warnf("SetMessageBoxes InsertOne error:%s", err.Error())
			return err
		}
		_ = result
	} else {
		...
		col, err := mgo.GetMongoDB().Database(message.DBMessage).Collection(message.TableMessageBox, op).InsertMany(ctx, update)
		if err != nil {
			log.Warnf("SetMessageBoxes InsertMany error:%s", err.Error())
			return err
		}
		_ = col
	}
