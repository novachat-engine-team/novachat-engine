...
var defaultMapCodec = bsoncodec.NewMapCodec()
...
func newDefaultStructCodec() *StructCodec {
	codec, err := NewStructCodec(bsoncodec.DefaultStructTagParser)
	if err != nil {
...
func NewStructCodec(p bsoncodec.StructTagParser, opts ...*bsonoptions.StructCodecOptions) (*StructCodec, error) {
	if p == nil {
...
func mapEncodeValue(mc *bsoncodec.MapCodec, ec bsoncodec.EncodeContext, dw bsonrw.DocumentWriter, val reflect.Value, collisionFn func(string) bool) error {
...
func encodeKey(mc *bsoncodec.MapCodec, val reflect.Value) (string, error) {
	...
	if km, ok := val.Interface().(bsoncodec.KeyMarshaler); ok {
...
func (sc *StructCodec) EncodeValue(r bsoncodec.EncodeContext, vw bsonrw.ValueWriter, val reflect.Value) error {
	if !val.IsValid() || val.Kind() != reflect.Struct {
		return bsoncodec.ValueEncoderError{Name: "StructCodec.EncodeValue", Kinds: []reflect.Kind{reflect.Struct}, Received: val}
	}
	...
	for _, desc := range sd.fl {
		...
		ectx := bsoncodec.EncodeContext{Registry: r.Registry, MinSize: desc.minSize}
	...
	if sd.inlineMap >= 0 {
		...
		return mapEncodeValue(defaultMapCodec, r, dw, rv, collisionFn)
	}
...
func (sc *StructCodec) DecodeValue(r bsoncodec.DecodeContext, vr bsonrw.ValueReader, val reflect.Value) error {
	...
	for {
		...
		dctx := bsoncodec.DecodeContext{Registry: r.Registry, Truncate: fd.truncate || r.Truncate}
		if fd.decoder == nil {
